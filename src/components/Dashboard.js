  ...
  const loadDashboardData = async () => {
    try {
      ...
      const lowStockItems = inventory.filter(
        (item) =>
          (item.godown_stock || 0) + (item.counter_stock || 0) <=
      ...
      const todayCash = todaySales
        .filter(
          (s) => s.paymentMethod === 'cash' || s.payment_method === 'cash'
        )
        .reduce(
          (sum, sale) =>
            sum + Number(sale.totalAmount || sale.total_amount || 0),
          0
        );
      ...
      recentSales.sort(
        (a, b) =>
          new Date(b.saleDate || b.sale_date) -
          new Date(a.saleDate || a.sale_date)
      );
    ...
    <div className="dashboard">
      ...
      <div className="summary-cards">
        ...
        <div className="summary-card card-mauve">
          ...
          <div className="value">
            ₹{dashboardData.todaySpendings.toFixed(0)}
          </div>
        </div>
          ...
          >
            ...
            >
              ...
              <span style={{ fontWeight: '800', fontSize: '1.05rem' }}>
                ₹{dashboardData.todayCash.toFixed(0)}
              </span>
            </div>
            <div
              style={{
            ...
            >
              <span>UPI:</span>
              <span style={{ fontWeight: '800', fontSize: '1.05rem' }}>
                ₹{dashboardData.todayUpi.toFixed(0)}
              </span>
            </div>
          </div>
        </div>
