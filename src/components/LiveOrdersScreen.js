...
import { Clock, Smartphone, Monitor, AlertCircle, MapPin, Check } from 'lucide-react';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '300px 1fr', gap: '16px', alignItems: 'start' }}>
              ...
              {orders.map((order) => {
                ...
                return (
                  ...
                  <div
                    ...
                    style={{
                      background: isDelivery ? '#faf5ff' : (isCompleted ? '#f3f4f6' : '#ffffff'),
                      borderRadius: '10px',
                      border: isSelected
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        {isDelivery && (
                          <span style={{ display: 'flex', alignItems: 'center', gap: '2px', background: '#f3e8ff', color: '#7e22ce', padding: '1px 6px', borderRadius: '4px', fontSize: '0.65rem', fontWeight: '700' }}>
                            🛵 Delivery
                          </span>
                        )}
                        {isWeb ? (
                    ...
                    <div style={{
                      fontSize: '0.8rem',
                      color: '#221f1a',
                      background: isDelivery ? '#f3e8ff' : (isCompleted ? '#e9ecef' : '#fbf7f4'),
                      padding: '6px 8px',
                      borderRadius: '6px',
                      border: `1px solid ${isDelivery ? '#e9d5ff' : '#f2e7db'}`,
                    }}>
                      <div style={{ fontWeight: '700', fontSize: '0.75rem', color: isDelivery ? '#7e22ce' : '#7f766a', marginBottom: '4px', borderBottom: `1px solid ${isDelivery ? '#e9d5ff' : '#f2e7db'}`, paddingBottom: '2px' }}>
                        Items:
                      </div>
                      ...
                      <div style={{ borderTop: `1px solid ${isDelivery ? '#e9d5ff' : '#f2e7db'}`, marginTop: '4px', paddingTop: '4px', display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', fontWeight: '700' }}>
                        <span>Total:</span>
                        <span style={{ color: isDelivery ? '#7e22ce' : '#b6412c' }}>{formatCurrency(order.totalAmount)}</span>
                      </div>
                    </div>
                    ...
                    <div style={{
                      ...
                      background: isDelivery ? '#7e22ce' : (isCompleted ? '#7f766a' : (order.orderStatus === 'pending_acceptance' ? '#e2a106' : '#1c8d3c')),
                    }} />
                  </div>

                  {/* Tick button — stays sharp above the blur */}
                  <button
                    onClick={(e) => { e.stopPropagation(); toggleTick(order.id); }}
                    ...
                    style={{
                      ...
                      padding: 0,
                      transition: 'all 0.2s',
                      boxShadow: isTicked ? '0 2px 8px rgba(22,163,74,0.35)' : '0 1px 4px rgba(0,0,0,0.12)',
                    }}
                  >
                    {isTicked && <Check size={13} strokeWidth={3} color="#ffffff" />}
                  </button>
                  </div>
                );
              })}
          ...
          {selectedOrder && (!isMobile || selectedOrder) && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1.5px solid #f2e7db', paddingBottom: '12px', marginBottom: '16px' }}>
                <div>
                  {isMobile && (
                    <button
                      ...
                      style={{ background: 'none', border: 'none', color: '#b6412c', fontWeight: '700', fontSize: '0.9rem', cursor: 'pointer', padding: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: '4px' }}
                    >
                      ← Back to Orders
                    </button>
                  )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                  <span style={{
                    ...
                    fontSize: '0.75rem',
                    fontWeight: '700',
                    background: selectedOrder.orderStatus === 'completed' ? '#dcfce7' : (selectedOrder.orderStatus === 'preparing' ? '#dbeafe' : '#fef3c7'),
                    color: selectedOrder.orderStatus === 'completed' ? '#15803d' : (selectedOrder.orderStatus === 'preparing' ? '#1d4ed8' : '#b45309'),
                  }}>
                    {selectedOrder.orderStatus === 'completed' ? 'Completed' : selectedOrder.orderStatus === 'preparing' ? 'Preparing' : 'Pending'}
                  </span>
              ...
              {/* Customer Info */}
              <div style={{ background: '#fbf7f4', border: '1px dashed #e6ded3', borderRadius: '8px', padding: '10px', marginBottom: '12px', fontSize: '0.88rem' }}>
                <h4 style={{ margin: '0 0 6px 0', fontSize: '0.8rem', color: '#7f766a', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Customer</h4>
              ...
              {selectedOrder.orderType === 'delivery' && (
                <div style={{ background: '#faf5ff', border: '2px solid #c084fc', borderRadius: '10px', padding: '14px', marginBottom: '16px' }}>
                  <h4 style={{ margin: '0 0 8px 0', fontSize: '0.85rem', color: '#7e22ce', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <MapPin size={14} /> Delivery Address
                  </h4>
                  {selectedOrder.deliveryAddress ? (
                    <div style={{ color: '#221f1a', lineHeight: '1.9', fontSize: '0.92rem' }}>
                    ...
                    <div style={{ color: '#9ca3af', fontStyle: 'italic', fontSize: '0.88rem' }}>No address provided</div>
                  )}
                </div>
              )}
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', border: '1.5px solid #f2e7db', borderRadius: '8px', overflow: 'hidden' }}>
              ...
              {/* Bill Totals */}
              <div style={{ borderTop: '2px solid #e6ded3', paddingTop: '12px' }}>
